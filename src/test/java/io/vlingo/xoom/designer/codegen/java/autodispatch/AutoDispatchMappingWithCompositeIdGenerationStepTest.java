...
public class AutoDispatchMappingWithCompositeIdGenerationStepTest extends CodeGenerationTest {
  ...
  public void testThatAutoDispatchMappingsAreGenerated() {
    ...
    final CodeGenerationParameter useAnnotationParameter = CodeGenerationParameter.of(Label.USE_ANNOTATIONS, "true");
    final CodeGenerationParameter useAutoDispatchParameter = CodeGenerationParameter.of(Label.USE_AUTO_DISPATCH, "true");
    ...
    final CodeGenerationParameters parameters = CodeGenerationParameters.from(packageParameter, dialect,
        useCQRSParameter, useAnnotationParameter, useAutoDispatchParameter, catalogAggregate(), nameValueObject());
    ...
    final Content catalogMappingContent = context.findContent(JavaTemplateStandard.AUTO_DISPATCH_MAPPING, "CatalogResource");
    final Content catalogHandlersMappingContent = context.findContent(JavaTemplateStandard.AUTO_DISPATCH_HANDLERS_MAPPING, "CatalogResourceHandlers");

    Assertions.assertEquals(10, context.contents().size());
    Assertions.assertTrue(catalogMappingContent.contains(TextExpectation.onJava().read("catalog-auto-dispatch-handlers-mapping-with-composite-id")));
    Assertions.assertTrue(catalogHandlersMappingContent.contains(TextExpectation.onJava().read("catalog-auto-dispatch-mapping-with-composite-id")));
  }
