...
public class AutoDispatchMappingGenerationStepTest extends CodeGenerationTest {
  ...
  public void testThatAutoDispatchMappingsAreGenerated() {
    final CodeGenerationParameters parameters =
        CodeGenerationParameters.from(CodeGenerationParameter.of(Label.PACKAGE, "Io.Vlingo.Xoomapp"),
            CodeGenerationParameter.of(Label.CQRS, true), CodeGenerationParameter.of(Label.DIALECT, Dialect.C_SHARP),
    ...
    final Content authorMappingContent = context.findContent(CsharpTemplateStandard.AUTO_DISPATCH_MAPPING, "AuthorResource");
    final Content authorHandlersMappingContent = context.findContent(CsharpTemplateStandard.AUTO_DISPATCH_HANDLERS_MAPPING, "AuthorResourceHandlers");

    Assertions.assertEquals(11, context.contents().size());
    Assertions.assertTrue(authorMappingContent.contains(TextExpectation.onCSharp().read("author-dispatch-mapping")));
    Assertions.assertTrue(authorHandlersMappingContent.contains(TextExpectation.onCSharp().read("author-dispatch-handlers-mapping")));
  }
  ...
  public void testThatAggregateWithoutFactoryMethodAutoDispatchMappingsAreGenerated() {
    final CodeGenerationParameters parameters =
        CodeGenerationParameters.from(CodeGenerationParameter.of(Label.PACKAGE, "Io.Vlingo.Xoomapp"),
            CodeGenerationParameter.of(Label.CQRS, true), CodeGenerationParameter.of(Label.DIALECT, Dialect.C_SHARP),
            authorAggregateWithoutFactoryMethod(), nameValueObject(), rankValueObject(), tagValueObject());
    ...
    Assertions.assertEquals(11, context.contents().size());
    Assertions.assertEquals(((TextBasedContent) authorMappingContent).text, (TextExpectation.onCSharp().read("author-dispatch-mapping")));
    Assertions.assertEquals(((TextBasedContent) authorHandlersMappingContent).text, (TextExpectation.onCSharp().read("author-dispatch-handlers-mapping")));
  }
  ...
  private Content[] contents() {
    return new Content[]{
        ...
        Content.with(CsharpTemplateStandard.AGGREGATE, new OutputFile(Paths.get(MODEL_PACKAGE_PATH, "author").toString(), "AuthorEntity.cs"), null, null, AUTHOR_AGGREGATE_CONTENT_TEXT),
        ...
        Content.with(CsharpTemplateStandard.VALUE_OBJECT, new OutputFile(Paths.get(MODEL_PACKAGE_PATH).toString(), "Tag.cs"), null, null, TAG_VALUE_OBJECT_CONTENT_TEXT),
        ...
        Content.with(CsharpTemplateStandard.QUERIES_ACTOR, new OutputFile(Paths.get(PERSISTENCE_PACKAGE_PATH).toString(), "AuthorQueriesActor.cs"), null, null, AUTHOR_QUERIES_ACTOR_CONTENT_TEXT),
    };
  }
