...
public class Queries {
  ...
  public static List<Queries> from() {
      return Collections.emptyList();
  }
  public static List<Queries> from(final List<Content> contents, final List<TemplateData> templatesData) {
    return from(Model.QUERY, contents, templatesData);
  }

  public static List<Queries> from(final Model model, final List<Content> contents, final List<TemplateData> templatesData) {
    if (!model.isQueryModel()) {
      return Collections.emptyList();
    }

    if (ContentQuery.exists(CsharpTemplateStandard.QUERIES_ACTOR, contents)) {
      return ContentQuery.filterByStandard(CsharpTemplateStandard.QUERIES_ACTOR, contents)
    ...
    return templatesData.stream().filter(data -> data.hasStandard(CsharpTemplateStandard.QUERIES_ACTOR))
  ...
  private Queries(final String protocolQualifiedName, final String actorQualifiedName) {
  ...
  private Queries(final String protocolPackageName, final String protocolName,
                  final String actorPackageName, final String actorName) {
