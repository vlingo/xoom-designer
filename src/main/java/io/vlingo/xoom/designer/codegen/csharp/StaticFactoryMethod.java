...
import static io.vlingo.xoom.designer.codegen.csharp.MethodScope.INSTANCE;
import static io.vlingo.xoom.designer.codegen.csharp.MethodScope.STATIC;
...
public class StaticFactoryMethod {
  ...
  private StaticFactoryMethod(final CodeGenerationParameter parent, final Arguments staticFactoryMethodArguments) {
    this.dataObjectName = CsharpTemplateStandard.DATA_OBJECT.resolveClassname(parent.value);
  ...
  private String resolveMethodParameters(final CodeGenerationParameter parent, final Arguments staticFactoryMethodArguments) {
    if (staticFactoryMethodArguments.isSingleArg()) {
      final String carrier = resolveCarrierName(parent);
      return String.format("%s %s", carrier, Introspector.decapitalize(carrier));
    }
    return Formatters.Arguments.DATA_OBJECT_CONSTRUCTOR.format(parent);
  }

  private String resolveConstructorInvocation(final CodeGenerationParameter parent, final Arguments staticFactoryMethodArguments) {
    if (staticFactoryMethodArguments.isSingleArg()) {
  ...
  private List<String> resolveValueObjectInitializers(final CodeGenerationParameter parent, final Arguments staticFactoryMethodArguments) {
  ...
  private String resolveCarrierName(final CodeGenerationParameter parent) {
    if (parent.isLabeled(Label.AGGREGATE)) {
      return CsharpTemplateStandard.AGGREGATE_STATE.resolveClassname(parent.value);
    }
  ...
  private String resolveSingleArgumentName(final CodeGenerationParameter parent, final Arguments staticFactoryMethodArguments) {
    if (staticFactoryMethodArguments.isSingleArg()) {
