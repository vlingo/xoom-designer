...
public class RestResource {
  ...
  public static List<RestResource> from(final List<Content> contents) {
    ...
    final Set<String> classNames = ContentQuery.findClassNames(contents, CsharpTemplateStandard.REST_RESOURCE);
    ...
    return IntStream.range(0, classNames.size())
        .mapToObj(index -> new RestResource(codeElementFormatter, iterator.next(), index, classNames.size()))
        .collect(toList());
  }

  private RestResource(final CodeElementFormatter codeElementFormatter, final String restResourceName,
                       final int resourceIndex, final int numberOfResources) {
