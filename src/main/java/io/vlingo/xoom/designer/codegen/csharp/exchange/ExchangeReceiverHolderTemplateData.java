...
public class ExchangeReceiverHolderTemplateData extends TemplateData {
  ...
  private Set<String> resolveImports(final CodeGenerationParameter exchange, final List<ExchangeReceiver> receiversParameters,
    ...
    if (involvesActorLoad) {
      ...
      imports.add(ContentQuery.findFullyQualifiedClassName(CsharpTemplateStandard.AGGREGATE, aggregateEntityName, contents));
      imports.add(Definition.class.getCanonicalName());
    }

    imports.add(ContentQuery.findFullyQualifiedClassName(CsharpTemplateStandard.AGGREGATE_PROTOCOL, AggregateDetail.resolveProtocolNameFor(exchange.parent()), contents));
  ...
  public TemplateStandard standard() {
    return CsharpTemplateStandard.EXCHANGE_RECEIVER_HOLDER;
  }
