...
public class ExchangeDispatcherTemplateData extends TemplateData {
  ...
  public static TemplateData from(final String exchangePackage,
    ...
    final List<CodeGenerationParameter> producerExchanges = aggregates
  ...
  private ExchangeDispatcherTemplateData(final String exchangePackage, final List<CodeGenerationParameter> producerExchanges,
                                         final List<Content> contents) {
  ...
  private Set<String> resolveImports(final List<CodeGenerationParameter> producerExchanges, final List<Content> contents) {
    return producerExchanges.stream().flatMap(exchange -> exchange.retrieveAllRelated(Label.DOMAIN_EVENT))
        .map(event -> ContentQuery.findFullyQualifiedClassName(CsharpTemplateStandard.DOMAIN_EVENT, event.value, contents))
  ...
  public TemplateStandard standard() {
    return CsharpTemplateStandard.EXCHANGE_DISPATCHER;
  }
